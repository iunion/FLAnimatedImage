...
// add by DJ
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options progress:(SDWebImageProgressBlock)progress success:(SDWebImageSuccessBlock)success failure:(SDWebImageFailureBlock)failure
{
...
{
    [self downloadWithURL:url delegate:delegate options:options userInfo:nil progress:nil success:success failure:failure];
}

// add by DJ
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo progress:(SDWebImageProgressBlock)progress success:(SDWebImageSuccessBlock)success failure:(SDWebImageFailureBlock)failure
{
    ...
    if (progress)
    {
        ...
        options = options | SDWebImageProgressiveShow;
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    UIImage *sImage = nil;
    FLAnimatedImage *sAnimatedImage = nil;
    ...
    if (sAnimatedImage)
    {
        if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithFLAnimatedImage:forURL:userInfo:)])
        {
            ...
            ((void ( *)(id, SEL, id, id, id, id))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithFLAnimatedImage:forURL:userInfo:), self, sAnimatedImage, url, userInfo);
        }
    }
    else if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
    {
        NSDictionary *userInfo = [info objectForKey:@"userInfo"];
        if ([userInfo isKindOfClass:NSNull.class])
        {
    ...
    if ([info objectForKey:@"animatedsuccess"])
    {
        ...
        success(image, sAnimatedImage, YES);
    }
#endif
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    // add by DJ
    if ((options & SDWebImageProgressiveShow) && !downloader.progressiveShow)
    {
        downloader.progressiveShow = YES;
    }
...
// add by DJ
- (void)imageDownloader:(SDWebImageDownloader *)downloader receivedSize:(NSUInteger)receivedSize expectedSize:(long long)expectedSize
{
    NSMutableArray *notifiedDelegates = [NSMutableArray arrayWithCapacity:downloaders.count];
...
#if NS_BLOCKS_AVAILABLE
            if ([info objectForKey:@"progress"])
            {
                SDWebImageProgressBlock progress = [info objectForKey:@"progress"];
            ...
            if (sImage)
            {
                ...
                if (sAnimatedImage)
                {
                    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithFLAnimatedImage:forURL:userInfo:)])
                    {
                        ...
                        ((void ( *)(id, SEL, id, id, id, id))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithFLAnimatedImage:forURL:userInfo:), self, sAnimatedImage, downloader.url, userInfo);
                    }
                }
                else if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
                {
